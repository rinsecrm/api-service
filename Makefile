...
help:
	...
	@echo "  build        - Build the HTTP API server"
...
build-ci:
	@echo "Building HTTP API server for CI with version info..."
	@mkdir -p bin
	@if [ -n "$(VERSION)" ] && [ -n "$(COMMIT)" ]; then \
		echo "Building with version: $(VERSION)-$(COMMIT)"; \
		go build -ldflags="-X main.version=$(VERSION) -X main.buildCommit=$(COMMIT) -X main.buildTime=$(shell date -u +%Y-%m-%dT%H:%M:%SZ)" -o bin/api-service ./cmd/server; \
	else \
...
run: build
	@echo "Running HTTP API server locally..."
	@echo "Make sure DynamoDB is running with: make dev-db"
	@STORE_SERVICE_ADDR=localhost:8081 ./bin/api-service

# Run the server locally without building (faster iteration)
dev-run:
	@echo "Running HTTP API server with go run..."
	@echo "Make sure DynamoDB is running with: make dev-db"
	@STORE_SERVICE_ADDR=localhost:8081 go run ./cmd/server
...
docker-build-ci:
	...
	@echo "Make sure to run 'make build' first to create bin/api-service"
	docker build -f Dockerfile.ci -t api-service:latest .
